...
noises = [0.001, 0.01, 0.1, 0.5, 1.0, 1.5, 2.0]
...
cost = []
convergence = []
transform = numpy.zeros((1575, 4))
f = open('workfile', 'w')
idx = 0
s =  'i convergence cost x y z yaw fx fy fz fyaw'
f.write(s)
for noise in noises:
  for x in xs:
    for y in ys:
      for z in zs:
        for yaw in yaws:
          ...
          print idx, convergence[idx], cost[idx], x, y, z, yaw, transform[idx][0], transform[idx][1], transform[idx][2], transform[idx][3]
          s = idx, convergence[idx], cost[idx], x, y, z, yaw, transform[idx][0], transform[idx][1], transform[idx][2], transform[idx][3]
